...
export let machine_location = [-1, 65, -21];
...
export function setMachineLocation(x, y, z) {
  machine_location = [x, y, z];
}

function isWithinRadius(x, y, z, radius) {
  if (machine_location.length !== 3) {
...
export function registerWith(vm) {
  ...
  vm.registerOpCode("STORAGE_OPEN", (args, context) => {
    ...
    if (dimension === "none") {
      throw new Error("Storage library dimension not initialized. Call setDimension() first.");
    }
    ...
    if (!isWithinRadius(coords.x, coords.y, coords.z, 10)) {
      throw new Error(`Container at (${coords.x}, ${coords.y}, ${coords.z}) is outside allowed radius of 10 blocks`);
    }
    
    const containerName = args[4];
    const block = world.getDimension(dimension).getBlock(coords);
    
    if (!block) {
      throw new Error(`No block found at coordinates (${coords.x}, ${coords.y}, ${coords.z})`);
    }
    ...
    const container = new Container(containerName, block);
    storage.set(containerName, container);
  });
